...
execute as @a at @s if entity @e[type=armor_stand,tag=dungeon_restriction,distance=..150] store success score @s event_triggered run fill ~-10 ~-10 ~-10 ~10 ~10 ~10 air replace #minecraft:beds

# If the event_triggered score is 1, it means a bed was broken and we can notify the player
execute as @a[scores={event_triggered=1..}] run tellraw @s {"text":"This place feels too spooky to sleep here","color":"dark_purple","italic":true}

# Return the broken bed (just gray for now)
execute as @a[scores={event_triggered=1..}] run give @p minecraft:gray_bed
